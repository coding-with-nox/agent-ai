        ...
        CommandChain chain, CancellationToken ct)
    {
        ...
        for (int i = 0; i < chain.Segments.Count; i++)
        {
            ...
            steps.Add(new PlanStep
            {
                ...
                DependsOn = deps,
                Status = NocodeX.Core.Enums.TaskStatus.Pending
            });
        ...
        ExecutionPlan plan, CancellationToken ct)
    {
        ...
        foreach (PlanStep step in plan.Steps)
        {
            ...
            bool depsReady = step.DependsOn.All(depId =>
                ...
                dep.Status == NocodeX.Core.Enums.TaskStatus.Success);

            if (!depsReady)
            {
                step.Status = NocodeX.Core.Enums.TaskStatus.Blocked;
                step.Error = "Blocked by failed dependencies";
                _logger.LogWarning("Step {StepId} blocked by dependencies", step.StepId);
                continue;
            }

            step.Status = NocodeX.Core.Enums.TaskStatus.Pending;
            ...
            try
            {
                ...
                step.Status = NocodeX.Core.Enums.TaskStatus.Success;
                await Task.Delay(10, ct); // yield for cancellation
            }
            catch (Exception ex)
            {
                step.Status = NocodeX.Core.Enums.TaskStatus.Failed;
                step.Error = ex.Message;
                _logger.LogError(ex, "Step {StepId} failed", step.StepId);
            }
        }

        bool allSuccess = plan.Steps.All(s => s.Status == NocodeX.Core.Enums.TaskStatus.Success);
        int succeeded = plan.Steps.Count(s => s.Status == NocodeX.Core.Enums.TaskStatus.Success);
