...
namespace Codex.Infrastructure.Configuration;

/// <summary>
/// Root configuration model for the CODEX agent.
/// </summary>
public sealed class CodexConfiguration
{
    ...
    /// </summary>
    /// <param name="filePath">Path to codex.config.json.</param>
    /// <param name="logger">Optional logger.</param>
    /// <returns>Parsed configuration.</returns>
    public static CodexConfiguration Load(string filePath, ILogger? logger = null)
    {
        if (!File.Exists(filePath))
        {
            logger?.LogWarning("Config file not found at {Path}, using defaults", filePath);
            return new CodexConfiguration();
        }

        string json = File.ReadAllText(filePath);
        CodexConfiguration? config = JsonSerializer.Deserialize<CodexConfiguration>(json, JsonOpts);
        return config ?? new CodexConfiguration();
    }
